...
5. Olvasd el az engine packet `retrieval.confidence`, `channels`, `preference_reranker`, `evidence_policy`, `selected_evidence`, `selection_trace`, `conflicts` és `evidence_compiler` részét. Az `evidence_policy.state == manual_policy_cold_start` kézzel kurált szabályrendszert jelent, nem tanult preference-et; `cold-start` rerankernél se állíts tanult preferenciát. Minden nem üres `conflicts` listát fel kell oldani release előtt, mert az ütköző guardokat az Engine kizárja. `low` confidence esetén ne állíts erős egyedi hangbizonyítékot: támogatott műfaji sheetre támaszkodj, hiányos sheetnél pedig kérj célzott mintát. Az email-sheet 100 legutóbbi elküldött levélből származó 67 érdemi, névtelenített jelvektorra épül. A Slack-sheet 800 saját üzenetből 788 érdemi, szövegmásolat nélkül tárolt jelre épül; csak másodlagos dialóguskalibrációként használd. Segíthet bizonytalan válaszritmusban, ellenvetésben, feedbackben, megszólításban és cselekvési lezárásban, de közvetlen műfaji evidence-et nem írhat felül, vershez vagy dalhoz pedig nem ad képi, rím- vagy zenei autoritást.
